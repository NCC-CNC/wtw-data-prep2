...
create_wtw_metadata <- function(project_dir, natdata_dir) {
  ...
  for (i in seq_along(tif_list)) {
    ...
    if (grepl("species/eccc_ch", tif_path)) {
      wtw_species_meta_row <- dplyr::filter(
        wtw_species_meta$ECCC_CH,
        File == tif_name
      )
      wtw_prep_meta_row <- dplyr::filter(
        wtw_prep_meta,
        path == "themes/species/eccc_ch"
      )
    } else if (grepl("species/eccc_sar", tif_path)) {
      wtw_species_meta_row <- dplyr::filter(
        wtw_species_meta$ECCC_SAR,
        File == tif_name
      )
      wtw_prep_meta_row <- dplyr::filter(
        wtw_prep_meta,
        path == "themes/species/eccc_sar"
      )
    } else if (grepl("species/iucn_amph", tif_path)) {
      wtw_species_meta_row <- dplyr::filter(
        wtw_species_meta$IUCN_AMPH,
        File == tif_name
      )
      wtw_prep_meta_row <- dplyr::filter(
        wtw_prep_meta,
        path == "themes/species/iucn_amph"
      )
    } else if (grepl("species/iucn_bird", tif_path)) {
      wtw_species_meta_row <- dplyr::filter(
        wtw_species_meta$IUCN_BIRD,
        File == tif_name
      )
      season <- stringr::str_extract(tif_base, "(?<=_)[0-9]+(?=_)")
      wtw_prep_meta_row <- dplyr::filter(
        wtw_prep_meta,
        short_name == paste0("IUCN_BIRD_S", season)
      )
    } else if (grepl("species/iucn_mamm", tif_path)) {
      wtw_species_meta_row <- dplyr::filter(
        wtw_species_meta$IUCN_MAMM,
        File == tif_name
      )
      wtw_prep_meta_row <- dplyr::filter(
        wtw_prep_meta,
        path == "themes/species/iucn_mamm"
      )
    } else if (grepl("species/iucn_rept", tif_path)) {
      wtw_species_meta_row <- dplyr::filter(
        wtw_species_meta$IUCN_REPT,
        File == tif_name
      )
      wtw_prep_meta_row <- dplyr::filter(
        wtw_prep_meta,
        path == "themes/species/iucn_rept"
      )
    } else if (grepl("species/nsc_end", tif_path)) {
      wtw_species_meta_row <- dplyr::filter(
        wtw_species_meta$NSC_END,
        File == tif_name
      )
      wtw_prep_meta_row <- dplyr::filter(
        wtw_prep_meta,
        path == "themes/species/nsc_end"
      )
    } else if (grepl("species/nsc_sar", tif_path)) {
      wtw_species_meta_row <- dplyr::filter(
        wtw_species_meta$NSC_SAR,
        File == tif_name
      )
      wtw_prep_meta_row <- dplyr::filter(
        wtw_prep_meta,
        path == "themes/species/nsc_sar"
      )
    } else if (grepl("species/nsc_spp", tif_path)) {
      wtw_species_meta_row <- dplyr::filter(
        wtw_species_meta$NSC_SPP,
        File == tif_name
      )
      wtw_prep_meta_row <- dplyr::filter(
        wtw_prep_meta,
        path == "themes/species/nsc_spp"
      )
    } else {
      wtw_species_meta_row <- NULL
      wtw_prep_meta_row <- dplyr::filter(
        wtw_prep_meta,
        wtw_file_name == tif_name
      )
    }
